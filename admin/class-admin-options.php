...
class Lazy_Load_For_Videos_Admin {
	...
	function lazyload_replace_video($return, $data, $url) {
	    ...
	    if ( (! is_feed()) && ($data->provider_name == 'YouTube')
			...
			$preview_url = "<a class=\"{$a_class}\" href=\"{$url}\" data-video-title=\"{$data->title}\" title=\"{$play_title_text}\">{$data->title}</a>";
	    ...
	    elseif ( (! is_feed()) && ($data->provider_name == 'Vimeo')
			...
			// $preview_img = "<img loading=\"lazy\" src=\"{$data->thumbnail_url}\" alt=\"\">";
			$preview_url = "<a href=\"{$url}\" id=\"{$vimeoid}\" class=\"{$a_class}\" data-video-thumbnail=\"{$data->thumbnail_url}\" data-video-title=\"{$data->title}\" title=\"{$play_title_text}\">{$data->title}</a>";
